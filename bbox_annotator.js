...
this.BBoxAnnotator = class BBoxAnnotator {
  ...
  constructor(options) {
    ...
    this.image_frame = $('<div class="image_frame" style="position:relative; left: 1000px"></div>');
    this.annotator_element.append(this.image_frame);
    if (options.guide) {
      annotator.initialize_guide(options.guide);
    }
    if(typeof(image_element)!="undefined"){
      $('.image_frame').remove();
      this.image_frame = $('<div class="image_frame" style="position: relative; left: 1000px"></div>');
      this.annotator_element.append(this.image_frame);
    }
    ...
    image_element.onload = function() {
      var MAX_WIDTH = 500;//adjust max picture width or height
      var MAX_HEIGHT = 500;
      ...
      //   "overflow": "hidden"
      // });
      annotator.annotator_element.css({//adjust area size!!!
        "width":'550px',
        "height": '550px',
        "padding-left": (annotator.border_width / 2 ) + 'px',
        "padding-top": (annotator.border_width / 2) + 'px',
      ...
      annotator.image_frame.css({
        "background": "url('" + image_element.src + "')",
        "width": width+"px",
        "height": height + "px",
        "position": "relative",
    ...
    image_element.onerror = function() {
      return annotator.annotator_element.text("\xa0".repeat(100));
    };
    this.entries = [];
  ...
  predict(e) {
    ...
    selector.start(200, 200);
    selector.update_rectangle(400, 400);
    alert("dd");
    selector.input_label(options);
